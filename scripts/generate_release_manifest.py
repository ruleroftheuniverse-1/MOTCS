...
def run():
    before=protected_hashes(ROOT);gates=accepted_gates()
    provenance_audit=json.loads(next((ROOT/"outputs/provisional/provenance_portability/run_018").glob("*audit.json")).read_text(encoding="utf-8"))
    gates["Run 018 provenance"]="RUN_018_CROSS_PLATFORM_PROVENANCE_GO" if provenance_audit["audit_status"]=="PROVENANCE_PATH_AUDIT_OK" else "RUN_018_CROSS_PLATFORM_PROVENANCE_REFINEMENT_REQUIRED"
    catalog=build_artifact_catalog(ROOT);ledger=authorization_ledger(gates,release_ready=True)
    ...
    semantic=ReleaseSemanticManifest(RELEASE_SCHEMA_VERSION,project["name"],"control-infrastructure-release-run-018",_commit(),semantic_hash(tuple((item.path,item.sha256) for item in catalog.artifacts)),project["version"],project["requires-python"],
        ...
        {"molecular_model":"mgf-mot-molecular-model-v1","control_policy":"mgf-mot-control-policy-v2","compiled_schedule":"mgf-mot-compiled-control-schedule-v1","open_loop_family":"mgf-mot-open-loop-policy-family-v1","feedback_session":"mgf-mot-feedback-session-v1","experiment":"mgf-mot-experiment-spec-v1","provenance_path_audit":"mgf-mot-provenance-path-audit-v1","release":RELEASE_SCHEMA_VERSION},
    ...
    environment={"schema_version":"mgf-mot-release-environment-v1","generation_time_utc":datetime.now(timezone.utc).isoformat(),"operating_system":sys.platform,"python_implementation":sys.implementation.name,"python_version":sys.version.split()[0],"direct_dependencies":{"PyYAML":">=6.0","pylcp":"==1.0.2"},"test_extra_dependencies":{"pytest":">=8.0","Pillow":">=10.0","pdfplumber":">=0.11.0","tomli":">=2.0; python_version < '3.11'"},"installed_versions":{name:_version(name) for name in ("mgf-mot-force-map","pylcp","numpy","scipy","PyYAML","pytest","Pillow","pdfplumber","tomli","build","setuptools","wheel")},"environment_kind":"AUDIT_SNAPSHOT_NOT_A_UNIVERSAL_LOCKFILE","labels":RELEASE_LABELS}
